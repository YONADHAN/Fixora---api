...
import {
  CustomError as AppError,
  CustomError,
} from '../../../domain/utils/custom.error'
...
export class InitiateChatUseCase implements IInitiateChatUseCase {
  ...
  async execute(data: InitiateChatDto): Promise<string> {
    ...
    if (!customer || !vendor || !service) {
      throw new AppError(
        'Related entities (Customer, Vendor, or Service) not found',
    ...
    if (!customerObjectId || !vendorObjectId || !serviceObjectId) {
      throw new AppError(
        'Invalid entity data: Missing UUIDs',
    ...
    console.log('Problem test 1')
    console.log(
      'The customer vendor service objectIds : ',
      customerObjectId,
      vendorObjectId,
      serviceObjectId,
    )
    ...
    if (!existingChat) {
      throw new CustomError(
        'Chat not yet available. Please wait a moment.',
        HttpStatus.CONFLICT,
      )
    }
