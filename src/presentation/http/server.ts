...
export class ExpressServer {
  ...
  private configureRoutes(): void {
    ...
    const vendorController =
      container.resolve<IVendorController>('IVendorController')
    const customerController = container.resolve<ICustomerController>(
      'ICustomerController'
    )
    const serviceCategoryController =
      container.resolve<IServiceCategoryController>(
        'IServiceCategoryController'
      )
    const blockMyUserMiddleware = container.resolve<IBlockMyUserMiddleware>(
      'IBlockMyUserMiddleware'
    )

    // Attach routes
    this._app.use('/api/v1/auth', new AuthRoutes(authController).router)
    ...
    this._app.use(
      '/api/v1/vendor',
      new VendorRoutes(authController, vendorController, blockMyUserMiddleware)
        .router
    )
